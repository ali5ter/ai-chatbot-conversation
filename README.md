...
# Define roles
policy_expert = "You are a practical AI policy expert."
ethics_researcher = "You are an AI ethics researcher."
...
conv.run_conversation(
    ...
    num_turns=3,  # Number of back-and-forth exchanges. Defaults to 5
    delay=1,      # Delay in seconds between API calls. Defaults to 1
    verbose=True  # Show conversation in the console. Default to True
)
...
The [perform.py](perform.py) script can perform the resulting output file generated by `chatbot_conversation.py` using these TTS modes:
...
Copy the voice‑mapping template:

```bash
cp voices.json.template voices.json
```

Edit the file to assign voices:
...
Set your API key if you want to use OpenAI modes:

```bash
export OPENAI_API_KEY="your-api-key"
```

### 2. Running a Performance

Choose one of the three playback modes.

#### macOS Mode (fastest, zero latency)

Uses macOS built‑in TTS voices such as *Alex, Daniel, Fiona, Moira, Karen, Tessa,* etc.

```bash
python perform.py conversation.txt --mode macos
```

#### OpenAI Live Mode (stream + immediate playback)

```bash
python perform.py conversation.txt --mode openai-live
```

#### OpenAI MP3 Mode (podcast‑style single audio file)

Generates an MP3 file called `final_performance.mp3`:

```bash
python perform.py conversation.txt --mode openai-mp3
```

**Note: This mode requires `ffmpeg`. Make sure it's installed, For example, on MacOS, you could install using `brew install ffmpeg`.**

### 3. Dynamic Voice Assignment
...
To override, edit the `voices.json` file to maps the voices you want, e.g.
